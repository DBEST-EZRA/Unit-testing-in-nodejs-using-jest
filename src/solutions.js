...
function toCamelCase(name, uppercase) {
  ...
  const camelCases = chunks.map((chunk, index) => {
...
function createLinkTag(rel, href) {
  const removeWhiteRelSpace = rel.trim();
  const removeWhiteHrefSpace = href.trim();

  const specialCharRegex = /[\s\t\n]/;

  const relValue = specialCharRegex.test(removeWhiteRelSpace)
    ? `"${removeWhiteRelSpace}"`
    : removeWhiteRelSpace;
  const hrefValue = specialCharRegex.test(removeWhiteHrefSpace)
    ? `"${removeWhiteHrefSpace}"`
    : removeWhiteHrefSpace;

  return `<link rel=${relValue} href=${hrefValue}>`;
}
...
function parseDateString(value) {
  ...
  const expectedDatePattern = /^(\d{4})\/(\d{2})\/(\d{2})|(\d{2})\/(\d{2})\/(\d{4})$/;

  const checkMatching = value.match(expectedDatePattern);

  if (!checkMatching) {
    throw new Error('Kindly comply to these Date Formats YYYY/MM/DD or DD/MM/YYYY');
  }

  const [, yyyy1, mm1, dd1, dd2, mm2, yyyy2] = checkMatching;

  const year = yyyy1 ? parseInt(yyyy1) : parseInt(yyyy2);
  const month = yyyy1 ? parseInt(mm1) - 1 : parseInt(mm2) - 1;
  const day = yyyy1 ? parseInt(dd1) : parseInt(dd2);

  if (year < 1000 || year > 9999) {
    throw new Error('Enter a correct year!');
  }
  if (month < 0 || month > 11) {
    throw new Error('Enter a correct month!');
  }
  if (day < 1 || day > 31) {
    throw new Error('Enter a correct day!');
  }
...
function toDateString(value, format) {
  ...
  if (!validDateFormats.includes(format)) {
    throw new Error('use either of these formats "YYYY/MM/DD", "DD/MM/YYYY" or "MM/DD/YYYY"');
  }
...
function normalizeTime(value) {
  const Hours12System = /^(\d{1,2}):(\d{2}):(\d{2})\s*(AM|PM)$/i;
  const Hours24System = /^(\d{2}):(\d{2}):(\d{2})$/;

  let matching, hours, minutes, seconds;

  if ((matching = value.match(Hours12System))) {
    [hours, minutes, seconds] = [
      parseInt(matching[1], 10),
      parseInt(matching[2], 10),
      parseInt(matching[3], 10)
    ];
    const period = matching[4].toUpperCase();
  ...
  } else if ((matching = value.match(Hours24System))) {
    [hours, minutes, seconds] = [
      parseInt(matching[1], 10),
      parseInt(matching[2], 10),
      parseInt(matching[3], 10)
...
function formatTimes(...values) {
  const validTime = values.map((time) => normalizeTime(time)).filter((time) => time !== null);
...
function buildApiEndpoint(resourceType, resourceId, extraData) {
  if (
    ...
    (extraData !== undefined && extraData.trim() === '')
  ) {
